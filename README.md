...
2. **قرار الموديل النهائي**: `OPENROUTER_MODEL` مثبت على `anthropic/claude-sonnet-4.5` كقرار إنتاج نهائي — مش نقطة تجربة. اتقرر كده لأن التطبيق طبي وحساس، والفرق في التكلفة مقابل موديلات أرخص وغير مجربة (زي `deepseek/deepseek-v4-flash-vision-exp` اللي لسه "Experimental" من DeepSeek نفسها) مش يستاهل المخاطرة في مرحلة الإطلاق. أي تغيير للموديل ده لازم يمر بمقارنة دقة فعلية على عينة روشتات حقيقية الأول، مش تغيير مباشر في `.env`.
