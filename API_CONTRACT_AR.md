...
## 8. قرار الموديل المثبت

خدمة الذكاء الاصطناعي متظبطة تشتغل بـ **`anthropic/claude-sonnet-4.5`** (عبر OpenRouter) في الإنتاج. ده قرار متعمد بعد ما قارناه فعليًا ببدائل أرخص/مجانية (موديل Gemini مجاني وموديل رؤية من DeepSeek) — Claude كان أدق بشكل ملحوظ في قراءة الروشتات بخط اليد، ومحتاجش أي حلول ترقيعية لعتبات الأمان زي ما احتاجت البدائل. متغيّرش الموديل المتظبط من غير ما تعيد المقارنة دي؛ موديل أرخص بيقرا جرعة غلط مش فعلاً أرخص.

ده شفاف تمامًا بالنسبة لـ ASP.NET Core: شكل العقد (الطلب/الاستجابة، أسماء الحقول، أكواد الأخطاء) مبيتغيرش بناءً على أي موديل متظبط تحت الغطاء. بس حقل `model_provider` في `health` هو اللي بيعكسه.
